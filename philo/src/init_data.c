...
int	init_philos(t_data *data)
{
	...
	while (++i < data->nb_philos)
	{
		...
		pthread_mutex_init(&philos[i].mutex_nb_meals_had, NULL);
...
int	init_data(t_data *data, int argc, char **argv)
{
	...
	pthread_mutex_init(&data->mutex_eat_t, NULL);
	pthread_mutex_init(&data->mutex_sleep_t, NULL);
	pthread_mutex_init(&data->mutex_die_t, NULL);
